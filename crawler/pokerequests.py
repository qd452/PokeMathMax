...
def get_favor_poke_ids(pokejson="favourite_poke.json"):
    ...
    """
    with open(os.path.join(FILE, 'favourite_poke.json'),
...
def getDataFromSGPokeMap():
    ...
    with requests.Session() as s:
        s.headers = {  # request headers setting
                       ...
                       "method": "GET",
                       "path": "/query2.php?since=" + str(
                           current_epoch_time) + "&mons=" + list_of_pokemon_ids,
                       "scheme": "https",
                       "accept": "*/*",
        ...
        r = s.get("https://sgpokemap.com/query2.php?since=" + str(
            current_epoch_time) + "&mons=" + list_of_pokemon_ids,
                  verify=False)
    ...
    if DEBUG:
        ...
        colorprint("In Total, {} Pokemons were found".format(
            len(response['pokemons'])), color="blue")
...
class PokeDict2Cls:
    ...
    def __repr__(self):
        if self.cp:
            pkstr = "{} - (IV:{}%) - (CP: {}) - (Level: {})".format(self.name,
                                                                    self.iv,
                                                                    self.cp,
                                                                    self.level)
            pkstr += "\nWeather Boost: {}".format(
                True if self.weather > 0 else False)
            pkstr += "\nL30+ Stats: ({}) {}".format(self.stats, self.iv)
            pkstr += "\nL30+ CP: {} (Level: {})".format(self.cp, self.level)
        else:
            pkstr = "{}".format(self.name)
        pkstr += "\nUntil: {} ({} left)".format(self.despawn, self.time_left)
...
def poke_detail_deserialization(**poke_info_dict):
    ...
    for k, v in poke_info_dict.items():
        if k == "despawn":
            poke_info_dict[k] = time.strftime("%I:%M:%S%p",
    ...
    for k, v in poke_info_dict.items():
        if k == "pokemon_id":
            try:
                ...
                raise PokeCrawlerError(
                    "Scanned Pokemon ID: {}is NOT in my favrouite list".format(
                        v))
        elif k == "attack":
    ...
    if poke_info_dict['stats'] == [None, None, None]:
        ...
        poke_info_dict["iv"] = round(sum(poke_info_dict['stats']) / 45.0 * 100,
                                     1)

    poke_obj = PokeDict2Cls(**poke_info_dict)
...
def main(**kwargs):
    ...
    colorprint(
        "\nBased on the current location {}, within radius of {}m, {} My Favourite Pokemons are found".format(
            loc, radius, len(pkms)), color="blue")
    for pkm in pkms:
        if pkm.cp:
            colorprint("===================================================",
                       color="red")
